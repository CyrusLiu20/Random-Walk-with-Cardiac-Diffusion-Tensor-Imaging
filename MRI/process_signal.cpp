...
process_signal::process_signal(std::vector<particle_state> &states, substrate &substrate_input, sequence &sequence_input, Eigen::MatrixXd &pos0){
    ...
    for (int i = 0; i < N_p; i++){
        ...
        Eigen::Vector3d temp = position_all(i, Eigen::all);
...
Eigen::Matrix3d process_signal::process_phase(Eigen::MatrixXd &phase, double &bvalue){
    ...
    for (int i = 0; i < ndirs; i++){
        Eigen::Vector3d dir = directions(i, Eigen::all); // [Gx, Gy, Gz]
...
void process_signal::process_displacement(Eigen::MatrixXd &displacement, double &T){
    ...
    Dx = (displacement(Eigen::all, 0).array()*displacement(Eigen::all, 0).array()).mean()/(2*T);
    Dy = (displacement(Eigen::all, 1).array()*displacement(Eigen::all, 1).array()).mean()/(2*T);
    Dz = (displacement(Eigen::all, 2).array()*displacement(Eigen::all, 2).array()).mean()/(2*T);
